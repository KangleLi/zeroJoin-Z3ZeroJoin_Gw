// ****************************************************************************
// * manufacturing-library-cli-host.c
// *
...
ALIGNMENT(2)
// carel (去除static属性，别处调用)
uint8_t   sendBuff[MAX_BUFFER_SIZE + 1];
// 20190719
                         ...
                         uint8_t *packetContents)
{
  ...
  // carel (第三方callback)
  uint8_t *__packet__ = (uint8_t *)malloc(packetLength + 1);
  __packet__[0] = packetLength;
  memcpy(__packet__+1, packetContents, packetLength);
 #if 1
  extern void _zeroJoinRxPacketCallBack(uint8_t *packet,
          uint8_t linkQuality,
          int8_t rssi);
 #endif
  _zeroJoinRxPacketCallBack(__packet__, linkQuality, rssi);
...
// carel (去除static属性)
EmberStatus sendPacket(uint8_t *buffer, uint16_t numPackets)
// 20190720
{
